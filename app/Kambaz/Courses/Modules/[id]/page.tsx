...
export default function ModulePage({ params }: ModulePageProps) {
  const modItem = moduleContent[params.id]; // renamed from 'module' to 'modItem'
  if (!modItem) {
    return <div style={{ padding: "1rem", color: "black" }}>Module not found.</div>;
  }
  return (
    <div style={{ padding: "1rem", color: "black" }}>
      <h1 style={{ fontWeight: "bold", fontSize: "2rem" }}>{modItem.title}</h1>
      <p style={{ fontSize: "1.1rem" }}>{modItem.content}</p>
    </div>
  );
}
